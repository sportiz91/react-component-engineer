    ...
    if choices:
        while True:
            ...
            if default:
                ...
                user_input = await session.prompt_async(f"Enter your choice (1-{len(choices)}) [default: {default_index}]: ", multiline=False)
                if not user_input and default:
                    return default
            else:
                user_input = await session.prompt_async(f"Enter your choice (1-{len(choices)}): ", multiline=False)
